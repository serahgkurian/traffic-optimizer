...
for episode in range(EPISODES):
    ...
    # New accumulators for statistics:
    total_queue_length = 0
    total_waiting_time = 0
    waiting_time_counts = 0
    total_throughput = 0
    ...
    for _ in range(MAX_PHASES_PER_EPISODE):
        ...
        for _ in range(duration):
            ...
            # Accumulate queue lengths
            queue_lengths = [traci.lane.getLastStepHaltingNumber(lane) for lane in CONTROLLED_LANES]
            total_queue_length += sum(queue_lengths)

            # Accumulate waiting times for vehicles on controlled lanes
            for lane in CONTROLLED_LANES:
                vehicles = traci.lane.getLastStepVehicleIDs(lane)
                for v in vehicles:
                    total_waiting_time += traci.vehicle.getWaitingTime(v)
                waiting_time_counts += len(vehicles)

        # Throughput: number of arrived vehicles at this point
        total_throughput = len(traci.simulation.getArrivedIDList())
    ...
    # Compute averages for the episode
    avg_queue_length = total_queue_length / step if step > 0 else 0
    avg_waiting_time = total_waiting_time / waiting_time_counts if waiting_time_counts > 0 else 0
    avg_throughput = total_throughput / step if step > 0 else 0  # throughput per step

    print(f"Episode {episode + 1}/{EPISODES} | Total Reward: {total_reward} | "
          f"Avg Queue Length: {avg_queue_length:.2f} | "
          f"Avg Waiting Time: {avg_waiting_time:.2f} sec | "
          f"Throughput (total arrived vehicles): {total_throughput}")
